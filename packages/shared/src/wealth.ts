...
export const WealthBriefingSchema = z.object({
  ...
  greeting: z.string(),
  summary: z.string(),
  scoreState: z.enum(["available", "insufficient_data"]),
  scoreExplanation: z.string(),
  scoreFactors: z.array(z.object({
    id: z.string(), available: z.boolean(), reason: z.string().optional(),
    originalWeight: z.number(), redistributedWeight: z.number().optional(), points: z.number().optional(),
  })),
  healthScore: z.number().int().min(0).max(100).nullable(),
  healthStatus: z.enum(["strong", "healthy", "watch", "urgent"]).nullable(),
