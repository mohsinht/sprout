...
const externalConnectorsEnabled = process.env.ENABLE_EXTERNAL_CONNECTORS === "true";
const structuredImportsEnabled = process.env.ENABLE_STRUCTURED_IMPORTS === "true";
...
if (isProduction && (!databaseUrl || databaseUrl.includes("sprout:sprout@localhost"))) {
  throw new Error("DATABASE_URL must use production database credentials in production");
}
...
if (
  isProduction &&
  corsOrigins.some((origin) => origin === "*" || /localhost|127\.0\.0\.1/.test(origin))
) {
  throw new Error("CORS_ORIGINS must list only deployed application origins in production");
}
...
if (isProduction && valuationExposureEnabled) {
  if (!process.env.VALUATION_BURN_IN_APPROVED_AT) {
    throw new Error("Real valuation exposure requires VALUATION_BURN_IN_APPROVED_AT");
  }
  if (navSource !== "al_meezan_validated" || fxSource !== "xe") {
    throw new Error("Real valuations require validated Al Meezan NAV and Xe FX sources");
  }
}
